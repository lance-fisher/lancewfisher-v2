...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-governance-pressure-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIGovernancePressureSmokeValidation.RunBatchAIGovernancePressureSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_GOVERNANCE_PRESSURE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_GOVERNANCE_PRESSURE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI governance pressure smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI governance pressure smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI governance pressure smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI governance pressure smoke produced no pass/fail marker. Check the log.'
exit 1
