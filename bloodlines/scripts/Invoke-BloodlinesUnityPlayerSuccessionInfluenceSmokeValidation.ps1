...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-succession-influence-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerSuccessionInfluenceSmokeValidation.RunBatchPlayerSuccessionInfluenceSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_SUCCESSION_INFLUENCE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_SUCCESSION_INFLUENCE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player succession influence smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player succession influence smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player succession influence smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player succession influence smoke produced no pass/fail marker. Check the log.'
exit 1
