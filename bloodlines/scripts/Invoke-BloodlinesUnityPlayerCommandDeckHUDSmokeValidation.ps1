...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-command-deck-hud-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerCommandDeckHUDSmokeValidation.RunBatchPlayerCommandDeckHUDSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_COMMAND_DECK_HUD_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_COMMAND_DECK_HUD_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player command-deck HUD smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player command-deck HUD smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player command-deck HUD smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player command-deck HUD smoke produced no pass/fail marker. Check the log.'
exit 1
