...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-missionary-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesMissionaryResolutionSmokeValidation.RunBatchMissionaryResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_MISSIONARY_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_MISSIONARY_RESOLUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity missionary resolution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Missionary resolution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Missionary resolution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Missionary resolution smoke produced no pass/fail marker. Check the log.'
exit 1
