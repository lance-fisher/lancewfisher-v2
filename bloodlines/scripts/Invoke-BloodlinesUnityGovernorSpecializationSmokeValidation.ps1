...
$logPath     = Join-Path $projectRoot 'artifacts\unity-governor-specialization-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesGovernorSpecializationSmokeValidation.RunBatchGovernorSpecializationSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_GOVERNOR_SPECIALIZATION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_GOVERNOR_SPECIALIZATION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity governor specialization smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Governor specialization smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Governor specialization smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Governor specialization smoke produced no pass/fail marker. Check the log.'
exit 1
