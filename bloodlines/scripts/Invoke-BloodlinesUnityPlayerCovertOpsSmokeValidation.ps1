...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-covert-ops-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerCovertOpsSmokeValidation.RunBatchPlayerCovertOpsSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_COVERT_OPS_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_COVERT_OPS_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player covert ops smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player covert ops smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player covert ops smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player covert ops smoke produced no pass/fail marker. Check the log.'
exit 1
