...
$logPath     = Join-Path $projectRoot 'artifacts\unity-patrol-route-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPatrolRouteSmokeValidation.RunBatchPatrolRouteSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PATROL_ROUTE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PATROL_ROUTE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity patrol route smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Patrol route smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Patrol route smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Patrol route smoke produced no pass/fail marker. Check the log.'
exit 1
