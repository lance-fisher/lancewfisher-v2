...
$logPath = Join-Path $rootPath 'artifacts\unity-breach-sealing-recovery-smoke.log'
...
function Get-ValidationOutcome {
    ...
    if ($content -match 'BLOODLINES_BREACH_SEALING_RECOVERY_SMOKE PASS') {
        return 'passed'
    }

    if ($content -match 'BLOODLINES_BREACH_SEALING_RECOVERY_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity breach sealing recovery smoke validation...'
...
if ($exitCode -eq 0 -and $outcome -eq 'unknown') {
    Write-Host 'First batch pass ended without an explicit breach sealing recovery outcome. Rerunning once after compilation/import.'
    $exitCode = Invoke-UnityValidationPass
    $outcome = Get-ValidationOutcome
}

if ($exitCode -eq 0 -and $outcome -ne 'passed') {
    throw "Unity exited with code 0 but breach sealing recovery smoke validation did not report an explicit pass. See $logPath"
}
