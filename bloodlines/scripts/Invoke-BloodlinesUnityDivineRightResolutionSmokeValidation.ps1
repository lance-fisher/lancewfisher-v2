...
$logPath     = Join-Path $projectRoot 'artifacts\unity-divine-right-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesDivineRightResolutionSmokeValidation.RunBatchDivineRightResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_DIVINE_RIGHT_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_DIVINE_RIGHT_RESOLUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity divine right resolution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Divine right resolution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Divine right resolution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Divine right resolution smoke produced no pass/fail marker. Check the log.'
exit 1
