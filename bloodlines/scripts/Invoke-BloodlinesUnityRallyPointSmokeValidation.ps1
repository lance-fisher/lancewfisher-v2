...
$logPath     = Join-Path $projectRoot 'artifacts\unity-rally-point-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesRallyPointSmokeValidation.RunBatchRallyPointSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_RALLY_POINT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_RALLY_POINT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity rally point smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Rally point smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Rally point smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Rally point smoke produced no pass/fail marker. Check the log.'
exit 1
