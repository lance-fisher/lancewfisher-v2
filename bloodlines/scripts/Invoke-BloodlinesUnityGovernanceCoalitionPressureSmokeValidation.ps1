...
$logPath     = Join-Path $projectRoot 'artifacts\unity-governance-coalition-pressure-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesGovernanceCoalitionPressureSmokeValidation.RunBatchGovernanceCoalitionPressureSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_GOVERNANCE_COALITION_PRESSURE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_GOVERNANCE_COALITION_PRESSURE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity governance coalition pressure smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Governance coalition pressure smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Governance coalition pressure smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Governance coalition pressure smoke produced no pass/fail marker. Check the log.'
exit 1
