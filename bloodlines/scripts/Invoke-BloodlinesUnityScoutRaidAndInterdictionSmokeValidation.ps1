...
$logPath     = Join-Path $rootPath 'artifacts\unity-scout-raid-and-interdiction-smoke.log'
...
$arguments = @(
    ...
    '-logFile',     $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesScoutRaidAndInterdictionSmokeValidation.RunBatchScoutRaidAndInterdictionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_SCOUT_RAID_AND_INTERDICTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_SCOUT_RAID_AND_INTERDICTION_SMOKE FAIL') { return 'failed' }
    return 'unknown'
}
...
Write-Host "Running Bloodlines Unity scout raid and interdiction smoke validation..."
...
if ($exitCode -eq 0 -and $outcome -ne 'passed') {
    throw "Unity exited 0 but scout raid and interdiction smoke did not report PASS. See $logPath"
}
