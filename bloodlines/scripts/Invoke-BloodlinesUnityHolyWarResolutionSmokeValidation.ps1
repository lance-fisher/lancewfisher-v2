...
$logPath     = Join-Path $projectRoot 'artifacts\unity-holy-war-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesHolyWarResolutionSmokeValidation.RunBatchHolyWarResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_HOLY_WAR_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_HOLY_WAR_RESOLUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity holy war resolution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Holy war resolution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Holy war resolution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Holy war resolution smoke produced no pass/fail marker. Check the log.'
exit 1
