...
$logPath     = Join-Path $projectRoot 'artifacts\unity-covenant-test-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesCovenantTestSmokeValidation.RunBatchCovenantTestSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_COVENANT_TEST_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_COVENANT_TEST_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity covenant test smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Covenant test smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Covenant test smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Covenant test smoke produced no pass/fail marker. Check the log.'
exit 1
