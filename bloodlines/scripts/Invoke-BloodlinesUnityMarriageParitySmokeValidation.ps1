...
$logPath     = Join-Path $rootPath 'artifacts\unity-marriage-parity-smoke.log'
...
$arguments = @(
    ...
    '-logFile',     $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesMarriageParitySmokeValidation.RunBatchMarriageParitySmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_MARRIAGE_PARITY_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_MARRIAGE_PARITY_SMOKE FAIL') { return 'failed' }
    return 'unknown'
}
...
Write-Host "Running Bloodlines Unity marriage parity smoke validation..."
...
if ($exitCode -eq 0 -and $outcome -ne 'passed') {
    throw "Unity exited 0 but marriage parity smoke did not report PASS. See $logPath"
}
