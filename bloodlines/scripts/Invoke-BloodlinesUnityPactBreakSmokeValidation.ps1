...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-pact-break-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPactBreakSmokeValidation.RunBatchPactBreakSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PACT_BREAK_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PACT_BREAK_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity pact break smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Pact break smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Pact break smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Pact break smoke produced no pass/fail marker. Check the log.'
exit 1
