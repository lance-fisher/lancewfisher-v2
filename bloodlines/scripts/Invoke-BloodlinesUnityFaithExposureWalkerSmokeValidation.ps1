...
$logPath = Join-Path $projectRoot 'artifacts\unity-faith-exposure-walker-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesFaithExposureWalkerSmokeValidation.RunBatchFaithExposureWalkerSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_FAITH_EXPOSURE_WALKER_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_FAITH_EXPOSURE_WALKER_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity faith exposure walker smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Faith exposure walker smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Faith exposure walker smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Faith exposure walker smoke produced no pass/fail marker. Check the log.'
exit 1
