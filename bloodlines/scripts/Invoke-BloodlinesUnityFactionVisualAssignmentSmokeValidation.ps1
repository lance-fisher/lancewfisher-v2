...
$logPath     = Join-Path $projectRoot 'artifacts\unity-faction-visual-assignment-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesFactionVisualAssignmentSmokeValidation.RunBatchFactionVisualAssignmentSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_FACTION_VISUAL_ASSIGNMENT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_FACTION_VISUAL_ASSIGNMENT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity faction visual assignment smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Faction visual assignment smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Faction visual assignment smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Faction visual assignment smoke produced no pass/fail marker. Check the log.'
exit 1
