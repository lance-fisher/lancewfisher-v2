...
$logPath     = Join-Path $projectRoot 'artifacts\unity-skirmish-game-mode-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesSkirmishGameModeSmokeValidation.RunBatchSkirmishGameModeSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_SKIRMISH_GAME_MODE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_SKIRMISH_GAME_MODE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity skirmish game mode smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Skirmish game mode smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Skirmish game mode smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Skirmish game mode smoke produced no pass/fail marker. Check the log.'
exit 1
