...
$logPath     = Join-Path $projectRoot 'artifacts\unity-faith-commitment-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesFaithCommitmentSmokeValidation.RunBatchFaithCommitmentSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_FAITH_COMMITMENT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_FAITH_COMMITMENT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity faith commitment smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Faith commitment smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Faith commitment smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Faith commitment smoke produced no pass/fail marker. Check the log.'
exit 1
