...
$logPath     = Join-Path $rootPath 'artifacts\unity-tier2-batch-smoke.log'
...
$arguments = @(
    ...
    '-logFile',     $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesTier2BatchSmokeValidation.RunBatchTier2BatchSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_TIER2_BATCH_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_TIER2_BATCH_SMOKE FAIL') { return 'failed' }
    return 'unknown'
}
...
Write-Host "Running Bloodlines Unity Tier 2 batch smoke validation..."
...
if ($exitCode -eq 0 -and $outcome -ne 'passed') {
    throw "Unity exited 0 but Tier 2 batch smoke did not report PASS. See $logPath"
}
