...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-captive-ransom-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerCaptiveRansomSmokeValidation.RunBatchPlayerCaptiveRansomSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_CAPTIVE_RANSOM_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_CAPTIVE_RANSOM_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player captive ransom smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player captive ransom smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player captive ransom smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player captive ransom smoke produced no pass/fail marker. Check the log.'
exit 1
