...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-covert-ops-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerCovertOpsResolutionSmokeValidation.RunBatchPlayerCovertOpsResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_COVERT_OPS_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_COVERT_OPS_RESOLUTION_SMOKE FAIL' -or
