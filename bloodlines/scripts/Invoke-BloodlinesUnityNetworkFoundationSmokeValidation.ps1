...
$logPath     = Join-Path $projectRoot 'artifacts\unity-network-foundation-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesNetworkFoundationSmokeValidation.RunBatchNetworkFoundationSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_NETWORK_FOUNDATION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_NETWORK_FOUNDATION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity network foundation smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Network foundation smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Network foundation smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Network foundation smoke produced no pass/fail marker. Check the log.'
exit 1
