...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-worker-gather-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIWorkerGatherSmokeValidation.RunBatchAIWorkerGatherSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_WORKER_GATHER_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_WORKER_GATHER_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI worker gather smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI worker gather smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI worker gather smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI worker gather smoke produced no pass/fail marker. Check the log.'
exit 1
