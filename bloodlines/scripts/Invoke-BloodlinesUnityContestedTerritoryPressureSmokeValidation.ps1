...
$logPath     = Join-Path $projectRoot 'artifacts\unity-contested-territory-pressure-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesContestedTerritoryPressureSmokeValidation.RunBatchContestedTerritoryPressureSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_CONTESTED_TERRITORY_PRESSURE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_CONTESTED_TERRITORY_PRESSURE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity contested territory pressure smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Contested territory pressure smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Contested territory pressure smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Contested territory pressure smoke produced no pass/fail marker. Check the log.'
exit 1
