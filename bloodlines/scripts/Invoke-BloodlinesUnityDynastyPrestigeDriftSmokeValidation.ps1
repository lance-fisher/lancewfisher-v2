...
$logPath     = Join-Path $projectRoot 'artifacts\unity-dynasty-prestige-drift-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesDynastyPrestigeDriftSmokeValidation.RunBatchDynastyPrestigeDriftSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_DYNASTY_PRESTIGE_DRIFT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_DYNASTY_PRESTIGE_DRIFT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity dynasty prestige drift smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Dynasty prestige drift smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Dynasty prestige drift smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Dynasty prestige drift smoke produced no pass/fail marker. Check the log.'
exit 1
