...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-pact-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerPactSmokeValidation.RunBatchPlayerPactSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_PACT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_PACT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player pact smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player pact smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player pact smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player pact smoke produced no pass/fail marker. Check the log.'
exit 1
