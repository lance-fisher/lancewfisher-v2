...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-covert-ops-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAICovertOpsSmokeValidation.RunBatchAICovertOpsSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_COVERT_OPS_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_COVERT_OPS_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI covert ops smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI covert ops smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI covert ops smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI covert ops smoke produced no pass/fail marker. Check the log.'
exit 1
