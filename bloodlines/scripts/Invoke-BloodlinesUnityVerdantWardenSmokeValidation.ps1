...
$logPath = Join-Path $projectRoot 'artifacts\unity-verdant-warden-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesVerdantWardenSmokeValidation.RunBatchVerdantWardenSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_VERDANT_WARDEN_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_VERDANT_WARDEN_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity Verdant Warden smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Verdant Warden smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Verdant Warden smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Verdant Warden smoke produced no pass/fail marker. Check the log.'
exit 1
