...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-command-dispatch-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAICommandDispatchSmokeValidation.RunBatchAICommandDispatchSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_COMMAND_DISPATCH_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_COMMAND_DISPATCH_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI command dispatch smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI command dispatch smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI command dispatch smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI command dispatch smoke produced no pass/fail marker. Check the log.'
exit 1
