...
$logPath     = Join-Path $projectRoot 'artifacts\unity-skirmish-status-hud-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesSkirmishStatusHUDSmokeValidation.RunBatchSkirmishStatusHUDSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_SKIRMISH_STATUS_HUD_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_SKIRMISH_STATUS_HUD_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity skirmish status HUD smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Skirmish status HUD smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Skirmish status HUD smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Skirmish status HUD smoke produced no pass/fail marker. Check the log.'
exit 1
