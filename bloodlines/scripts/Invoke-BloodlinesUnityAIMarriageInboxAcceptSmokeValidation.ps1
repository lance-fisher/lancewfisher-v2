...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-marriage-inbox-accept-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIMarriageInboxAcceptSmokeValidation.RunBatchAIMarriageInboxAcceptSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_MARRIAGE_INBOX_ACCEPT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_MARRIAGE_INBOX_ACCEPT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI marriage inbox accept smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI marriage inbox accept smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI marriage inbox accept smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI marriage inbox accept smoke produced no pass/fail marker. Check the log.'
exit 1
