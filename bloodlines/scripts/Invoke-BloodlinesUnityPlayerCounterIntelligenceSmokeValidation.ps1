...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-counter-intelligence-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerCounterIntelligenceSmokeValidation.RunBatchPlayerCounterIntelligenceSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_COUNTER_INTELLIGENCE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_COUNTER_INTELLIGENCE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player counter-intelligence smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player counter-intelligence smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player counter-intelligence smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player counter-intelligence smoke produced no pass/fail marker. Check the log.'
exit 1
