...
$logPath     = Join-Path $projectRoot 'artifacts\unity-nfe-integration-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesNfEIntegrationSmokeValidation.RunBatchNfEIntegrationSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_NFE_INTEGRATION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_NFE_INTEGRATION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity NfE integration smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'NfE integration smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "NfE integration smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'NfE integration smoke produced no pass/fail marker. Check the log.'
exit 1
