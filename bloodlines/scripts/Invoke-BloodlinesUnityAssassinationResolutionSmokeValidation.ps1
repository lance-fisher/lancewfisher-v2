...
$logPath     = Join-Path $projectRoot 'artifacts\unity-assassination-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAssassinationResolutionSmokeValidation.RunBatchAssassinationResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_ASSASSINATION_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_ASSASSINATION_RESOLUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity assassination resolution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Assassination resolution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Assassination resolution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Assassination resolution smoke produced no pass/fail marker. Check the log.'
exit 1
