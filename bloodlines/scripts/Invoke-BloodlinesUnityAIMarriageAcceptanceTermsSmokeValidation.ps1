...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-marriage-acceptance-terms-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIMarriageAcceptanceTermsSmokeValidation.RunBatchAIMarriageAcceptanceTermsSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_MARRIAGE_ACCEPTANCE_TERMS_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_MARRIAGE_ACCEPTANCE_TERMS_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI marriage acceptance terms smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI marriage acceptance terms smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI marriage acceptance terms smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI marriage acceptance terms smoke produced no pass/fail marker. Check the log.'
exit 1
