...
$logPath = 'D:\ProjectsHome\Bloodlines\artifacts\unity-bootstrap-scene-validate.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesGameplaySceneBootstrap.RunBatchValidateBootstrapSceneShell'
)
...
function Get-ValidationOutcome {
    ...
    if ($content -match 'Bootstrap scene shell validation passed') {
        return 'passed'
    }

    if ($content -match 'Bootstrap scene shell validation failed') {
        return 'failed'
    }
...
Write-Host "Running Bloodlines Unity Bootstrap scene validation..."
