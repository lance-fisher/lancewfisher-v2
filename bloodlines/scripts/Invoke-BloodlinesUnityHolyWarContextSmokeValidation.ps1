...
$logPath     = Join-Path $projectRoot 'artifacts\unity-holy-war-context-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesHolyWarContextSmokeValidation.RunBatchHolyWarContextSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_HOLY_WAR_CONTEXT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_HOLY_WAR_CONTEXT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity holy war context smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Holy war context smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Holy war context smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Holy war context smoke produced no pass/fail marker. Check the log.'
exit 1
