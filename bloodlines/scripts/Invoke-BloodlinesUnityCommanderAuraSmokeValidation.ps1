...
$logPath     = Join-Path $projectRoot 'artifacts\unity-commander-aura-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesCommanderAuraSmokeValidation.RunBatchCommanderAuraSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_COMMANDER_AURA_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_COMMANDER_AURA_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity commander aura smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Commander aura smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Commander aura smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Commander aura smoke produced no pass/fail marker. Check the log.'
exit 1
