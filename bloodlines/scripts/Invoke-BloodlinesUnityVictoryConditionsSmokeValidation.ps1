...
$logPath     = Join-Path $rootPath 'artifacts\unity-victory-conditions-smoke.log'
...
$arguments = @(
    ...
    '-logFile',     $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesVictoryConditionsSmokeValidation.RunBatchVictoryConditionsSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_VICTORY_CONDITIONS_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_VICTORY_CONDITIONS_SMOKE FAIL') { return 'failed' }
    return 'unknown'
}
...
Write-Host "Running Bloodlines Unity Victory Conditions smoke validation..."
...
if ($exitCode -eq 0 -and $outcome -ne 'passed') {
    throw "Unity exited 0 but Victory Conditions smoke did not report PASS. See $logPath"
}
