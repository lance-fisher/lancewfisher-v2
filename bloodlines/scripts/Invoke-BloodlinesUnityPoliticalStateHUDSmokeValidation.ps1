...
$logPath     = Join-Path $projectRoot 'artifacts\unity-political-state-hud-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPoliticalStateHUDSmokeValidation.RunBatchPoliticalStateHUDSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_POLITICAL_STATE_HUD_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_POLITICAL_STATE_HUD_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity political state HUD smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Political state HUD smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Political state HUD smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Political state HUD smoke produced no pass/fail marker. Check the log.'
exit 1
