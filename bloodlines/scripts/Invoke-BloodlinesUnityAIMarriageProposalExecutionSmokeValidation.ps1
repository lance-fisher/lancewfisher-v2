...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-marriage-proposal-execution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIMarriageProposalExecutionSmokeValidation.RunBatchAIMarriageProposalExecutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_MARRIAGE_PROPOSAL_EXECUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_MARRIAGE_PROPOSAL_EXECUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI marriage proposal execution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI marriage proposal execution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI marriage proposal execution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI marriage proposal execution smoke produced no pass/fail marker. Check the log.'
exit 1
