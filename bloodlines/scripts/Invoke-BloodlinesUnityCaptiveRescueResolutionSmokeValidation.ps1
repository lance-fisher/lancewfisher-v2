...
$logPath     = Join-Path $projectRoot 'artifacts\unity-captive-rescue-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesCaptiveRescueResolutionSmokeValidation.RunBatchCaptiveRescueResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_CAPTIVE_RESCUE_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_CAPTIVE_RESCUE_RESOLUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity captive rescue resolution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Captive rescue resolution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Captive rescue resolution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Captive rescue resolution smoke produced no pass/fail marker. Check the log.'
exit 1
