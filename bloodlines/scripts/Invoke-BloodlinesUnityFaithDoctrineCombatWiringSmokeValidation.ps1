...
$logPath     = Join-Path $projectRoot 'artifacts\unity-faith-doctrine-combat-wiring-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesFaithDoctrineCombatWiringSmokeValidation.RunBatchFaithDoctrineCombatWiringSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_FAITH_DOCTRINE_COMBAT_WIRING_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_FAITH_DOCTRINE_COMBAT_WIRING_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity faith doctrine combat wiring smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Faith doctrine combat wiring smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Faith doctrine combat wiring smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Faith doctrine combat wiring smoke produced no pass/fail marker. Check the log.'
exit 1
