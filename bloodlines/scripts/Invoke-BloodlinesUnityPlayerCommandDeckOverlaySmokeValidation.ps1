...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-command-deck-overlay-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerCommandDeckOverlaySmokeValidation.RunBatchPlayerCommandDeckOverlaySmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_COMMAND_DECK_OVERLAY_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_COMMAND_DECK_OVERLAY_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player command-deck overlay smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player command-deck overlay smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player command-deck overlay smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player command-deck overlay smoke produced no pass/fail marker. Check the log.'
exit 1
