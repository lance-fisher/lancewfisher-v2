...
$logPath    = Join-Path $rootPath 'artifacts\unity-ai-strategy-smoke.log'
...
$arguments = @(
    ...
    '-logFile',     $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIStrategySmokeValidation.RunBatchAIStrategySmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_STRATEGY_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_STRATEGY_SMOKE FAIL') { return 'failed' }
    return 'unknown'
}
...
Write-Host "Running Bloodlines Unity AI strategy smoke validation..."
...
if ($exitCode -eq 0 -and $outcome -ne 'passed') {
    throw "Unity exited 0 but AI strategy smoke did not report PASS. See $logPath"
}
