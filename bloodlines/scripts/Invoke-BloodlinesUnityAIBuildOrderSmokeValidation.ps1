...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-build-order-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIBuildOrderSmokeValidation.RunBatchAIBuildOrderSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_BUILD_ORDER_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_BUILD_ORDER_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI build order smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI build order smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI build order smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI build order smoke produced no pass/fail marker. Check the log.'
exit 1
