...
$logPath     = Join-Path $projectRoot 'artifacts\unity-dynasty-progression-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesDynastyProgressionSmokeValidation.RunBatchDynastyProgressionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_DYNASTY_PROGRESSION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_DYNASTY_PROGRESSION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity dynasty progression smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Dynasty progression smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Dynasty progression smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Dynasty progression smoke produced no pass/fail marker. Check the log.'
exit 1
