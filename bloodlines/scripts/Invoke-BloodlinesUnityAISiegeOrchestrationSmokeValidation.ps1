...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-siege-orchestration-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAISiegeOrchestrationSmokeValidation.RunBatchAISiegeOrchestrationSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_SIEGE_ORCHESTRATION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_SIEGE_ORCHESTRATION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI siege orchestration smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI siege orchestration smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI siege orchestration smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI siege orchestration smoke produced no pass/fail marker. Check the log.'
exit 1
