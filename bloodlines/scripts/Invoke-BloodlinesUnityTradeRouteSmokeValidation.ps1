...
$logPath     = Join-Path $projectRoot 'artifacts\unity-trade-route-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesTradeRouteSmokeValidation.RunBatchTradeRouteSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_TRADE_ROUTE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_TRADE_ROUTE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity trade route smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Trade route smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Trade route smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Trade route smoke produced no pass/fail marker. Check the log.'
exit 1
