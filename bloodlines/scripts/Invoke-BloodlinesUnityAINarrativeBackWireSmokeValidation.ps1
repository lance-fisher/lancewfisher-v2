...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-narrative-back-wire-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAINarrativeBackWireSmokeValidation.RunBatchAINarrativeBackWireSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_NARRATIVE_BACK_WIRE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_NARRATIVE_BACK_WIRE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI narrative back-wire smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI narrative back-wire smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI narrative back-wire smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI narrative back-wire smoke produced no pass/fail marker. Check the log.'
exit 1
