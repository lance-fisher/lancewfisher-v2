...
$logPath     = Join-Path $projectRoot 'artifacts\unity-succession-crisis-context-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesSuccessionCrisisContextSmokeValidation.RunBatchSuccessionCrisisContextSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_SUCCESSION_CRISIS_CONTEXT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_SUCCESSION_CRISIS_CONTEXT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity succession crisis context smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Succession crisis context smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Succession crisis context smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Succession crisis context smoke produced no pass/fail marker. Check the log.'
exit 1
