...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-divine-right-context-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerDivineRightContextSmokeValidation.RunBatchPlayerDivineRightContextSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_DIVINE_RIGHT_CONTEXT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_DIVINE_RIGHT_CONTEXT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player divine right context smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player divine right context smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player divine right context smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player divine right context smoke produced no pass/fail marker. Check the log.'
exit 1
