...
$logPath = Join-Path $projectRoot 'artifacts\unity-faith-structure-regen-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesFaithStructureRegenSmokeValidation.RunBatchFaithStructureRegenSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_FAITH_STRUCTURE_REGEN_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_FAITH_STRUCTURE_REGEN_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity faith structure regen smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Faith structure regen smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Faith structure regen smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Faith structure regen smoke produced no pass/fail marker. Check the log.'
exit 1
