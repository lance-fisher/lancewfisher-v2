...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-narrative-message-bridge-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesNarrativeMessageBridgeSmokeValidation.RunBatchNarrativeMessageBridgeSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_NARRATIVE_MESSAGE_BRIDGE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_NARRATIVE_MESSAGE_BRIDGE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity narrative message bridge smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Narrative message bridge smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Narrative message bridge smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Narrative message bridge smoke produced no pass/fail marker. Check the log.'
exit 1
