...
$logPath     = Join-Path $projectRoot 'artifacts\unity-fortification-hud-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesFortificationHUDSmokeValidation.RunBatchFortificationHUDSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_FORTIFICATION_HUD_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_FORTIFICATION_HUD_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity fortification HUD smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Fortification HUD smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Fortification HUD smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Fortification HUD smoke produced no pass/fail marker. Check the log.'
exit 1
