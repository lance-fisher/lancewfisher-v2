...
$logPath     = Join-Path $projectRoot 'artifacts\unity-victory-leaderboard-hud-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesVictoryLeaderboardHUDSmokeValidation.RunBatchVictoryLeaderboardHUDSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_VICTORY_LEADERBOARD_HUD_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_VICTORY_LEADERBOARD_HUD_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity victory leaderboard HUD smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Victory leaderboard HUD smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Victory leaderboard HUD smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Victory leaderboard HUD smoke produced no pass/fail marker. Check the log.'
exit 1
