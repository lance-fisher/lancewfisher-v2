...
$logPath     = Join-Path $projectRoot 'artifacts\unity-player-missionary-dispatch-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesPlayerMissionaryDispatchSmokeValidation.RunBatchPlayerMissionaryDispatchSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_PLAYER_MISSIONARY_DISPATCH_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_PLAYER_MISSIONARY_DISPATCH_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity player missionary dispatch smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Player missionary dispatch smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Player missionary dispatch smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Player missionary dispatch smoke produced no pass/fail marker. Check the log.'
exit 1
