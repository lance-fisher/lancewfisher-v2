...
$logPath = Join-Path $rootPath 'artifacts\unity-breach-sealing-worker-locality-smoke.log'
...
function Get-ValidationOutcome {
    ...
    if ($content -match 'BLOODLINES_BREACH_SEALING_WORKER_LOCALITY_SMOKE PASS') {
        return 'passed'
    }

    if ($content -match 'BLOODLINES_BREACH_SEALING_WORKER_LOCALITY_SMOKE FAIL' -or
...
if ($outcome -eq 'passed') {
    Write-Host "Unity exited with code $exitCode and breach sealing worker locality smoke reported PASS."
    exit 0
}

if ($outcome -eq 'unknown') {
    Write-Host 'First batch pass ended without an explicit breach sealing worker locality outcome. Rerunning once after compilation/import.'
    $exitCode = Invoke-UnityValidationPass
    $outcome = Get-ValidationOutcome
}
...
if ($outcome -eq 'failed') {
    Write-Host "Unity exited with code $exitCode and breach sealing worker locality smoke reported FAIL."
    exit 1
}

throw "Unity exited with code $exitCode but breach sealing worker locality smoke validation did not report an explicit outcome. See $logPath"
