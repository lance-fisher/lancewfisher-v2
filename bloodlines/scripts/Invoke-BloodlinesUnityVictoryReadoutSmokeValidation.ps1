...
$logPath     = Join-Path $projectRoot 'artifacts\unity-victory-readout-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesVictoryReadoutSmokeValidation.RunBatchVictoryReadoutSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_VICTORY_READOUT_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_VICTORY_READOUT_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity victory readout smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Victory readout smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Victory readout smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Victory readout smoke produced no pass/fail marker. Check the log.'
exit 1
