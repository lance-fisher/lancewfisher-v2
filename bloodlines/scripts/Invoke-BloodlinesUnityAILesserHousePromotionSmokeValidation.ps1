...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-lesser-house-promotion-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAILesserHousePromotionSmokeValidation.RunBatchAILesserHousePromotionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_LESSER_HOUSE_PROMOTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_LESSER_HOUSE_PROMOTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI lesser house promotion smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI lesser house promotion smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI lesser house promotion smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI lesser house promotion smoke produced no pass/fail marker. Check the log.'
exit 1
