...
$logPath     = Join-Path $projectRoot 'artifacts\unity-siege-escalation-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesSiegeEscalationSmokeValidation.RunBatchSiegeEscalationSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_SIEGE_ESCALATION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_SIEGE_ESCALATION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity siege escalation smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Siege escalation smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Siege escalation smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Siege escalation smoke produced no pass/fail marker. Check the log.'
exit 1
