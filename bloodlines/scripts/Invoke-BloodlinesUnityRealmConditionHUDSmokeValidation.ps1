...
$logPath     = Join-Path $projectRoot 'artifacts\unity-realm-condition-hud-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesRealmConditionHUDSmokeValidation.RunBatchRealmConditionHUDSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_REALM_CONDITION_HUD_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_REALM_CONDITION_HUD_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity realm-condition HUD smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Realm-condition HUD smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Realm-condition HUD smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Realm-condition HUD smoke produced no pass/fail marker. Check the log.'
exit 1
