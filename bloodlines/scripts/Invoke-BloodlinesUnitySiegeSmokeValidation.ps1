...
$logPath = Join-Path $rootPath 'artifacts\unity-siege-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesSiegeSmokeValidation.RunBatchSiegeSmokeValidation'
)
...
function Get-ValidationOutcome {
    ...
    if ($content -match 'Siege smoke validation passed') {
        return 'passed'
    }

    if ($content -match 'Siege smoke validation failed' -or
...
Write-Host 'Running Bloodlines Unity siege smoke validation...'
...
if ($exitCode -eq 0 -and $outcome -eq 'unknown') {
    Write-Host 'First batch pass ended without an explicit siege outcome. Rerunning once after compilation/import.'
    $exitCode = Invoke-UnityValidationPass
    $outcome = Get-ValidationOutcome
}
