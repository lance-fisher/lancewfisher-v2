...
$logPath     = Join-Path $projectRoot 'artifacts\unity-trueborn-rise-arc-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesTruebornRiseArcSmokeValidation.RunBatchTruebornRiseArcSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_TRUEBORN_RISE_ARC_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_TRUEBORN_RISE_ARC_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity Trueborn rise arc smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Trueborn rise arc smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Trueborn rise arc smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Trueborn rise arc smoke produced no pass/fail marker. Check the log.'
exit 1
