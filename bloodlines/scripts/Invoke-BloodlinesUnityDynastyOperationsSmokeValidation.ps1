...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-dynasty-operations-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesDynastyOperationsSmokeValidation.RunBatchDynastyOperationsSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_DYNASTY_OPERATIONS_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_DYNASTY_OPERATIONS_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity dynasty operations smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Dynasty operations smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Dynasty operations smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Dynasty operations smoke produced no pass/fail marker. Check the log.'
exit 1
