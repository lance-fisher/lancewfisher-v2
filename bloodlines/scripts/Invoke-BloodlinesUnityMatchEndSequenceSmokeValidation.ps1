...
$logPath     = Join-Path $projectRoot 'artifacts\unity-match-end-sequence-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesMatchEndSequenceSmokeValidation.RunBatchMatchEndSequenceSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_MATCH_END_SEQUENCE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_MATCH_END_SEQUENCE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity match end sequence smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Match end sequence smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Match end sequence smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Match end sequence smoke produced no pass/fail marker. Check the log.'
exit 1
