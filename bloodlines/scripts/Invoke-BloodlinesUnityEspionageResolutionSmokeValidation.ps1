...
$logPath     = Join-Path $projectRoot 'artifacts\unity-espionage-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesEspionageResolutionSmokeValidation.RunBatchEspionageResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_ESPIONAGE_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_ESPIONAGE_RESOLUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity espionage resolution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Espionage resolution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Espionage resolution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Espionage resolution smoke produced no pass/fail marker. Check the log.'
exit 1
