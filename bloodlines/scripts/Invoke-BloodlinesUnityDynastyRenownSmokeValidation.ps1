...
$logPath     = Join-Path $projectRoot 'artifacts\unity-dynasty-renown-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesDynastyRenownSmokeValidation.RunBatchDynastyRenownSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_DYNASTY_RENOWN_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_DYNASTY_RENOWN_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity dynasty renown smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Dynasty renown smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Dynasty renown smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Dynasty renown smoke produced no pass/fail marker. Check the log.'
exit 1
