...
$logPath     = 'D:\ProjectsHome\Bloodlines\artifacts\unity-ai-marriage-strategic-profile-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesAIMarriageStrategicProfileSmokeValidation.RunBatchAIMarriageStrategicProfileSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_AI_MARRIAGE_STRATEGIC_PROFILE_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_AI_MARRIAGE_STRATEGIC_PROFILE_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity AI marriage strategic profile smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'AI marriage strategic profile smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "AI marriage strategic profile smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'AI marriage strategic profile smoke produced no pass/fail marker. Check the log.'
exit 1
