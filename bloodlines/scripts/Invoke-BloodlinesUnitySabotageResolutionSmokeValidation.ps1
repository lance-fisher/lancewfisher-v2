...
$logPath     = Join-Path $projectRoot 'artifacts\unity-sabotage-resolution-smoke.log'
...
$arguments = @(
    ...
    '-logFile', $logPath
    '-executeMethod', 'Bloodlines.EditorTools.BloodlinesSabotageResolutionSmokeValidation.RunBatchSabotageResolutionSmokeValidation'
)

function Get-ValidationOutcome {
    if (-not (Test-Path -LiteralPath $logPath)) { return 'unknown' }
    $content = Get-Content -Path $logPath -Raw
    if ($content -match 'BLOODLINES_SABOTAGE_RESOLUTION_SMOKE PASS') { return 'passed' }
    if ($content -match 'BLOODLINES_SABOTAGE_RESOLUTION_SMOKE FAIL' -or
...
Write-Host 'Running Bloodlines Unity sabotage resolution smoke validation...'
...
if ($outcome -eq 'passed') {
    Write-Host 'Sabotage resolution smoke validation passed.'
    exit 0
}

if ($outcome -eq 'failed') {
    Write-Host "Sabotage resolution smoke validation FAILED. Unity exit code $exitCode"
    exit 1
}

Write-Host 'Sabotage resolution smoke produced no pass/fail marker. Check the log.'
exit 1
