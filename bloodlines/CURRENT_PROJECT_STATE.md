...
Last updated: 2026-04-20 (dynasty marriage parity slice complete on branch `codex/unity-dynasty-marriage-parity`: `MarriageProposalExpirationSystem` now matches the browser 90-day window, `MarriageGestationSystem` now matches the browser 280-day window and records `MarriageChildElement` plus `DynastyMixedBloodlineComponent`, new `MarriageDeathDissolutionSystem` ports death-driven dissolution with legitimacy and oathkeeping effects, dedicated 4-phase marriage parity smoke PASS, full governed validation chain green in `D:\BLDMP\bloodlines`, contract revision 43 -> 44)
