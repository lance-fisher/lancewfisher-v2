...
namespace Bloodlines.EditorTools
{
    public static class BloodlinesPlayerCaptiveRansomSmokeValidation
    {
        ...
        private static bool RunAllPhases(out string report)
        {
            ...
            ok &= RunHostileCaptorPhase(lines);
        ...
        private static bool RunSuccessPhase(System.Text.StringBuilder lines)
        {
            ...
            if (!debugScope.CommandSurface.TryDebugDispatchCaptiveRansom("player", captiveMemberId.ToString(), 70f))
            {
            ...
            if (!operation.TargetFactionId.Equals(new FixedString32Bytes("enemy")) ||
                ...
                !Approximately(resources.Influence, 62f) ||
                !Approximately(ransom.ResolveAtInWorldDays, 96f) ||
                messagesAfter - messagesBefore != 1 ||
                !Approximately(ransom.EscrowCost.Gold, 70f))
            {
                lines.AppendLine(
                    $"Phase 1 FAIL: invalid ransom state target={operation.TargetFactionId}, member={operation.TargetMemberId}, gold={resources.Gold}, influence={resources.Influence}, resolve={ransom.ResolveAtInWorldDays}, messages={messagesAfter - messagesBefore}, escrowGold={ransom.EscrowCost.Gold}.");
                return false;
            }

            lines.AppendLine(
                $"Phase 1 PASS: captive ransom created resolveAt={ransom.ResolveAtInWorldDays:0.##}, gold=100->30, influence=80->62.");
            return true;
        }
        ...
        private static bool RunInsufficientGoldPhase(System.Text.StringBuilder lines)
        {
            ...
            var playerFaction = SeedFaction(entityManager, "player", 69f, 80f);
            SeedFaction(entityManager, "enemy", 60f, 30f);
            var captiveMemberId = MarkPlayerCommanderCaptured(entityManager, playerFaction, "enemy");

            if (!debugScope.CommandSurface.TryDebugDispatchCaptiveRansom("player", captiveMemberId.ToString(), 70f))
            {
                lines.AppendLine("Phase 2 FAIL: could not queue insufficient-gold captive ransom request.");
                return false;
            }
            ...
            if (TryGetSingleOperation(entityManager, DynastyOperationKind.CaptiveRansom, "player", out _, out _) ||
                !Approximately(resources.Gold, 69f) ||
            ...
            lines.AppendLine("Phase 2 PASS: insufficient gold blocked ransom dispatch.");
            return true;
        }

        private static bool RunHostileCaptorPhase(System.Text.StringBuilder lines)
        {
            ...
            var captiveMemberId = MarkPlayerCommanderCaptured(entityManager, playerFaction, "enemy");
            PlayerPactUtility.EnsureHostility(entityManager, new FixedString32Bytes("player"), new FixedString32Bytes("enemy"));
            PlayerPactUtility.EnsureHostility(entityManager, new FixedString32Bytes("enemy"), new FixedString32Bytes("player"));

            if (!debugScope.CommandSurface.TryDebugDispatchCaptiveRansom("player", captiveMemberId.ToString(), 70f))
            {
                lines.AppendLine("Phase 3 FAIL: could not queue hostile-captor captive ransom request.");
                return false;
            }
            ...
            if (TryGetSingleOperation(entityManager, DynastyOperationKind.CaptiveRansom, "player", out _, out _) ||
                ...
                !Approximately(resources.Influence, 80f) ||
                !entityManager.HasBuffer<HostilityComponent>(enemyFaction))
            {
                lines.AppendLine("Phase 3 FAIL: hostile captor should block ransom dispatch and preserve resources.");
                return false;
            }

            lines.AppendLine("Phase 3 PASS: hostile captor gate blocked ransom dispatch.");
            return true;
        }
