...
- Last Updated By: claude-ai-missionary-resolution-2026-04-20
- Supersedes: revision 40 (Bundle 4 of the AI mechanical campaign lands on top of Claude's revision-40 ai-strategic-layer Bundle 3 captive rescue + ransom execution. Bundle 4 ships sub-slice 25 missionary resolution alone (single-sub-slice bundle because per-kind resolution systems differ substantively and should ship independently). Sub-slice 25 adds AIMissionaryResolutionSystem ([UpdateInGroup(SimulationSystemGroup), UpdateAfter(AIMissionaryExecutionSystem)]) which walks every DynastyOperationComponent entity with OperationKind=Missionary and Active=true, checks DynastyOperationMissionaryComponent.ResolveAtInWorldDays against current DualClock.InWorldDays, and on expiry applies the canonical browser missionary resolution effects from simulation.js:5517-5588 (tickDynastyOperations missionary branch) + simulation.js:10473-10503 (applyMissionaryEffect). Resolution semantics: success gate is SuccessScore >= 0 (deterministic, matching browser's gate at simulation.js:5527); on success applies ExposureGain to target's FaithExposureElement buffer (appending a new entry with Discovered=true if no prior exposure for the source faith; otherwise updating the existing entry; both paths clamp the final Exposure to [0, 100]), applies IntensityErosion to target's FaithStateComponent.Intensity (clamped >= 0) IFF target has a different committed faith than source (browser gate at simulation.js:10485-10488), and applies LoyaltyPressure to the lowest-loyalty ControlPointComponent owned by target (clamped [0, 100]); on failure applies +2 intensity reinforcement to target's FaithStateComponent.Intensity only when target has a committed faith (browser at simulation.js:5563-5566). Always flips DynastyOperationComponent.Active = false regardless of outcome so the operation slot releases back to the per-faction capacity budget (sub-slice 18). Narrative push via NarrativeMessageBridge.Push with three variants: success ("<faith> missionaries breach <target> with pressure dispatched by <source>."; tone Good when source=player, Warn when target=player, else Info), failure ("<target> repelled missionaries of <faith> from <source>."; tone Warn when source=player, Good when target=player, else Info), void ("<source> missionary mission expired without a valid target at <target>."; tone Info always). Unity-side simplifications (deferred to future slices): ward-profile-triggered mutual hostility on failure (browser simulation.js:5567; requires faith ward profile surface); conviction event recording for oathkeeping/desecration/stewardship +1 (requires ConvictionComponent mutation owned by the conviction-scoring lane); legitimacy-penalty fallback on success when no control points exist (browser adjustLegitimacy at simulation.js:10493-10495; requires the dynasty-core lane's DynastyStateComponent.Legitimacy mutation surface beyond what the current lane-scope permits; Unity falls back to a no-op when target owns zero control points). New BloodlinesMissionaryResolutionSmokeValidation 8-phase validator covers: PhaseNotExpiredSkips (ResolveAt > current, Active stays true, no effects, no narrative); PhaseExpiredSuccessApplies (Active->false, exposure 10->30, target intensity 50->45, loyalty 80->77, narrative +1); PhaseExpiredFailureStrengthens (Active->false, target intensity 50->52 via +2 reinforcement, exposure/loyalty unchanged); PhaseVoidOnMissingTarget (Active->false, void narrative pushed); PhaseExposureAppendsNewEntry (no prior entry -> new entry with Discovered=true, Exposure=20); PhaseExposureClampsAt100 (95+20 -> 100); PhaseLowestLoyaltyControlPointTargeted (two cps at 80 and 50; only 50 cp reduced to 47, 80 cp unchanged); PhaseIntensityErosionSkippedWhenSameFaith (matching SelectedFaith blocks intensity erosion; target intensity remains 50). Bundle 4 branched from revision-40 master (e73933e4) after Bundle 3 landed. No regressions to fortification-siege, Bundle 1/2/3, or sub-slice 8-18 surfaces; sub-slice 18 helpers (DynastyOperationLimits) gain a new consumer shape (the resolution-walker pattern) that does NOT call BeginOperation or HasCapacity but rather reads DynastyOperationComponent entities as a query surface, which is additive to the sub-slice 18 contract without reshaping existing producer callers. Sub-slice 20 DynastyOperationMissionaryComponent gains its first consumer. Sub-slice 16 NarrativeMessageBridge gains three new call sites.)
