...
- Last Updated By: codex-unity-fortification-breach-sealing-recovery-2026-04-19
- Supersedes: revision 38 (Codex fortification-siege sub-slice 8 breach sealing / recovery lands on top of Claude's revision-38 ai-strategic-layer Bundle 2 master base `cec33509`. The slice adds `BreachSealingProgressComponent` (per-settlement progress surface carrying `AccumulatedWorkerHours`, `StoneReservedForCurrentBreach`, and `LastTickInWorldDays`) plus `BreachSealingSystem` ([UpdateInGroup(SimulationSystemGroup)]) that gives defenders a Unity-native recovery loop for `OpenBreachCount`. Per fortified settlement with one or more open breaches, the system lazily attaches the progress component, reads the owning faction's stone reserve from `ResourceStockpileComponent`, scans that faction's live idle workers by `UnitTypeComponent.Role == Worker`, positive `HealthComponent`, and `WorkerGatherComponent.Phase == Idle`, reserves `60` stone for each breach as sealing starts, accumulates `8` in-world worker-hours per breach on a 1 Hz tick, and decrements `FortificationComponent.OpenBreachCount` when the active breach completes. No new worker-count field was added to `FortificationReserveComponent`; the direct idle-worker scan keeps the slice player/AI-neutral and avoids widening the reserve schema for a foundation-only recovery seam. Because sub-slice 5's `FortificationDestructionResolutionSystem` previously rewrote `OpenBreachCount = destroyedWalls + destroyedGates` every frame, this revision narrows that system so it still refreshes destroyed wall/tower/gate/keep counters but only adds newly-created breaches back into the live open-breach count, preserving sealed closures while destroyed structures remain unrepaired. New `BloodlinesBreachSealingRecoverySmokeValidation` covers six phases: half-window proportional progress with breaches unchanged, first breach sealed with `OpenBreachCount` dropping `2 -> 1` and stone `200 -> 140`, insufficient-stone block, zero-idle-workers block, no-open-breaches no-op with no progress component attached, and full sealing of three breaches with `3 * 60` stone consumed. New wrapper `scripts/Invoke-BloodlinesUnityBreachSealingRecoverySmokeValidation.ps1` proves the slice in batch mode. Bundle 2's revision-38 master state is preserved underneath this revision, and the fortification branch rebases onto that base before close.)
