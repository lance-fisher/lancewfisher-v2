...
- Last Updated By: claude-ai-captive-rescue-and-ransom-execution-2026-04-20
- Supersedes: revision 39 (Bundle 3 of the AI mechanical campaign lands on top of Codex's revision-39 fortification-siege sub-slice 8 breach sealing / recovery. Bundle 3 packages two ai-strategic-layer sub-slices into a single commit + merge: sub-slice 23 ports the captive rescue execution path as the fourth consumer of the sub-slice 18 dynasty-operations foundation and the first production reader of the sub-slice 19 CapturedMemberElement buffer, and sub-slice 24 ports the captive ransom execution path as the fifth consumer of the foundation. Sub-slice 23 adds DynastyOperationCaptiveRescueComponent (per-kind component carrying ResolveAtInWorldDays for the 20 in-world-day rescue duration, CaptiveMemberId, CaptiveMemberTitle, CaptorFactionId, SpymasterMemberId resolved from priority [Spymaster, Diplomat, Merchant], DiplomatMemberId resolved from priority [Diplomat, Merchant, HeirDesignate], HoldingSettlementId/KeepTier/WardId placeholders defaulting to empty/0 because the Unity holding-settlement and ward-profile surfaces are not yet ported, SuccessScore from simplified parity power - difficulty using base constants without renown scaling, ProjectedChance clamped 0.12-0.88 from 0.5 + (successScore / 45), IntensityCost 0 because rescue does not deduct intensity, and DynastyOperationEscrowCost with Gold = 42 + Influence = 26 from RESCUE_BASE_GOLD_COST and RESCUE_BASE_INFLUENCE_COST) plus AICaptiveRescueExecutionSystem ([UpdateInGroup(SimulationSystemGroup), UpdateAfter(AICovertOpsSystem)]) which consumes AICovertOpsComponent.LastFiredOp == CovertOpKind.CaptiveRescue, picks a Held captive belonging to source by walking every faction's CapturedMemberElement buffer (simplified Unity port of browser pickAiCaptiveRecoveryTarget at ai.js:3011 which sorts by role priority then renown; Unity port returns the first match because CapturedMemberElement does not yet carry roleId/renown), gates on (source faction exists, captive picker found a Held captive, no existing active dynasty operation for the captive's member id, source has Spymaster + Diplomat operatives on roster with non-Fallen/non-Captured status, source ResourceStockpileComponent.Gold >= 42, source ResourceStockpileComponent.Influence >= 26, DynastyOperationLimits.HasCapacity), deducts cost on success, computes simplified parity formula (power = 12 + spymaster.renown * 0.95 + diplomat.renown * 0.35; difficulty = 16 base; successScore = power - difficulty), calls DynastyOperationLimits.BeginOperation with DynastyOperationKind.CaptiveRescue + captor faction id as target + captive member id, attaches the per-kind component, and pushes a NarrativeMessageBridge.Push line ("<source> dispatches covert agents to recover <captive> from <captor>.") with tone Info when source is player and Good otherwise (browser tone routing simulation.js:11108). Sub-slice 24 adds DynastyOperationCaptiveRansomComponent (per-kind component carrying ResolveAtInWorldDays for the 16 in-world-day ransom duration, CaptiveMemberId, CaptiveMemberTitle, CaptorFactionId, DiplomatMemberId resolved from priority [Diplomat, Merchant, HeirDesignate, HeadOfBloodline], MerchantMemberId resolved from priority [Merchant, Governor, HeadOfBloodline], ProjectedChance hardcoded 1.0 because ransom is a paid transaction not a roll per browser simulation.js:4964, IntensityCost 0, and DynastyOperationEscrowCost with Gold = 70 + Influence = 18 from RANSOM_BASE_GOLD_COST and RANSOM_BASE_INFLUENCE_COST) plus AICaptiveRansomExecutionSystem (same group/order) which consumes AICovertOpsComponent.LastFiredOp == CovertOpKind.CaptiveRansom, reuses the same captive picker as the rescue system via three `internal static` helpers (TryPickCaptive, HasActiveOperationForMember, TryGetMemberByRolePriority) exposed on AICaptiveRescueExecutionSystem so the ransom system gets identical picker + gate semantics without duplication, gates on (source faction exists, captive picker found a Held captive, no existing active dynasty operation for the captive's member id, source has Diplomat + Merchant operatives on roster with non-Fallen/non-Captured status, source ResourceStockpileComponent.Gold >= 70, source ResourceStockpileComponent.Influence >= 18, DynastyOperationLimits.HasCapacity), deducts cost on success, calls DynastyOperationLimits.BeginOperation with DynastyOperationKind.CaptiveRansom + captor faction id as target + captive member id, attaches the per-kind component, pushes a narrative line ("<source> opens ransom terms for <captive> with <captor>.") with same tone routing (Info when source is player, Good otherwise). Both systems clear LastFiredOp to None unconditionally after processing (one-shot pattern shared with sub-slices 8/9/12/13/14/20/21/22). Browser duration translation continues the sub-slice 20 convention: RESCUE_BASE_DURATION_SECONDS = 20, RANSOM_BASE_DURATION_SECONDS = 16 are reinterpreted as in-world-day numeric values directly. Notable Unity-side simplifications: (1) cost calculation uses canonical base constants without browser's renown/role/keepTier/ward scaling because CapturedMemberElement does not yet carry renown or roleId; future captive-element extension can tighten cost calculation; (2) captive picker returns the first match found rather than sorting by role priority + renown; (3) holding-settlement keep-tier, ward-profile difficulty, and captor-faction spymaster contribution to rescue difficulty are deferred until the corresponding surfaces port; (4) per-kind resolution is intentionally deferred for both sub-slices following the Bundle 1/2 pattern. New BloodlinesCaptiveRescueAndRansomExecutionSmokeValidation 9-phase validator covers: PhaseRescueDispatchSuccess (op created with player target + captive member id, per-kind attached with ResolveAt = current+20, EscrowCost.Gold=42, EscrowCost.Influence=26, OperatorMemberIds resolved, narrative +1 Good, dispatch cleared), PhaseRescueNoCaptiveBlocks (no Held captives blocks), PhaseRescueNoSpymasterBlocks (only Commander on roster blocks), PhaseRescueInsufficientGoldBlocks (gold 30 < 42 blocks), PhaseRescueInsufficientInfluenceBlocks (influence 20 < 26 blocks), PhaseRescueExistingOperationBlocks (preexisting active op for member id blocks new dispatch), PhaseRansomDispatchSuccess (op created, per-kind attached with ResolveAt = current+16, EscrowCost.Gold=70, EscrowCost.Influence=18, ProjectedChance=1.0, narrative +1, dispatch cleared), PhaseRansomInsufficientGoldBlocks (gold 50 < 70 blocks), PhaseRansomNoMerchantBlocks (only Diplomat on roster blocks). Parallel-revision note: Bundle 3 originally branched from revision-38 master (cec33509) and claimed revision 39 concurrently with Codex's fortification sub-slice 8; Codex landed first at revision 39 on master 7821d74a, so Bundle 3 rebased onto 7821d74a and bumped to revision 40. No regressions to fortification-siege (sub-slice 8 BreachSealingSystem remains green), Bundle 1, Bundle 2, or sub-slice 8-18 surfaces; sub-slice 18 helpers (DynastyOperationLimits.HasCapacity / BeginOperation) gain their fourth and fifth production callers without reshape, and sub-slice 19 CapturedMemberElement buffer gains its first two production readers without reshape.)
...
- Current Branch In Flight: `claude/unity-ai-holy-war-and-divine-right-execution`
- Last Slice Handoff: `docs/unity/session-handoffs/2026-04-19-unity-ai-strategic-layer-bundle-2-holy-war-and-divine-right-execution.md`
...
- Status: active
- Branch Prefix: `codex/unity-fortification-breach-sealing-recovery` (sub-slice 8), `codex/unity-fortification-breach-legibility-readout` (sub-slice 7), `codex/unity-fortification-breach-assault-pressure` (sub-slice 6), `codex/unity-fortification-wall-segment-destruction` (sub-slice 5); prior `codex/unity-fortification-siege*` slices 1-4 also landed; future follow-ups should continue on fresh `codex/unity-fortification-*` branches
- Owner Agent: codex
- Owned Paths (exclusive):
...
- Current Branch In Flight: `codex/unity-fortification-breach-sealing-recovery`
- Last Slice Handoff: `docs/unity/session-handoffs/2026-04-19-unity-fortification-siege-breach-sealing-recovery.md`
...
Note: `fortification-siege-sub-slice-8-breach-sealing-recovery` is implemented on `codex/unity-fortification-breach-sealing-recovery` and documented in this revision. Further fortification follow-ups should continue using fresh `codex/unity-fortification-*` branches rather than widening the earlier fortification branches.
