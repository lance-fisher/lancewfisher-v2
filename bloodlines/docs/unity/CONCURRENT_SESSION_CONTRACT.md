...
- Last Updated By: claude-ai-holy-war-and-divine-right-execution-2026-04-19
- Supersedes: revision 37 (Bundle 2 of the AI mechanical campaign lands on top of Claude's revision-37 ai-strategic-layer Bundle 1 captive state + missionary execution. Bundle 2 packages two ai-strategic-layer sub-slices into a single commit + merge: sub-slice 21 ports the holy war declaration execution path as the second consumer of the sub-slice 18 dynasty-operations foundation, and sub-slice 22 ports the divine right declaration execution path as the third consumer. Sub-slice 21 adds DynastyOperationHolyWarComponent (per-kind component carrying ResolveAtInWorldDays for the 18 in-world-day declaration window, WarExpiresAtInWorldDays for the 180 in-world-day full holy-war duration, OperatorMemberId, OperatorTitle, IntensityPulse and LoyaltyPulse from doctrine bias [browser simulation.js:10468-10469: 1.2/0.9 dark/light intensity, 1.8/1.2 dark/light loyalty], CompatibilityLabel from a simplified Unity-side derivation, IntensityCost = 18, and DynastyOperationEscrowCost with Influence = 24) plus AIHolyWarExecutionSystem ([UpdateInGroup(SimulationSystemGroup), UpdateAfter(AICovertOpsSystem)]) which consumes AICovertOpsComponent.LastFiredOp == CovertOpKind.HolyWar, gates on getHolyWarDeclarationTerms parity (source != target, source FaithStateComponent.SelectedFaith != None, target faction exists with committed faith, simplified harmonious-tier check via identical (faith, doctrine), source faith operator on roster matching IdeologicalLeader/Spymaster/HeadOfBloodline/Diplomat with non-Fallen/non-Captured status, source ResourceStockpileComponent.Influence >= 24 HOLY_WAR_COST, source FaithStateComponent.Intensity >= 18 HOLY_WAR_INTENSITY_COST), capacity-gates on DynastyOperationLimits.HasCapacity, deducts cost on success, computes per-kind doctrine-bias pulses, calls DynastyOperationLimits.BeginOperation with DynastyOperationKind.HolyWar + player target, attaches the per-kind component, and pushes a NarrativeMessageBridge.Push line ("<source> sends a holy war declaration toward <target>.") with tone Warn when source or target is player and Info otherwise (browser tone routing simulation.js:10599 always-warn-on-player-involvement). Sub-slice 22 adds DynastyOperationDivineRightComponent (per-kind component carrying ResolveAtInWorldDays for the 180 in-world-day declaration window, SourceFaithId mapped from CovenantId, DoctrinePath captured at declaration, RecognitionShare/RecognitionSharePct placeholders defaulting to 0 because the global recognition share calculator is not yet ported, ActiveApexStructureId/ActiveApexStructureName placeholders defaulting to empty because the apex structure surface is not yet ported) plus AIDivineRightExecutionSystem (same group/order) which consumes AICovertOpsComponent.LastFiredOp == CovertOpKind.DivineRight, gates on the simplified Unity-side parity set (source FaithStateComponent.SelectedFaith != None, source FaithStateComponent.Intensity >= 80 DIVINE_RIGHT_INTENSITY_THRESHOLD, source FaithStateComponent.Level >= 5, no existing active DivineRight DynastyOperationComponent for this faction, DynastyOperationLimits.HasCapacity), calls DynastyOperationLimits.BeginOperation with DynastyOperationKind.DivineRight and default target (divine right has no specific target faction; affected factions are derived at resolution time), attaches the per-kind component, pushes a narrative line ("<source> declares Divine Right under <faith>. The spread window opens for 180 in-world days.") with tone Warn always (browser simulation.js:10832 always-warn). Both systems clear LastFiredOp to None unconditionally after processing (one-shot pattern shared with sub-slices 8/9/12/13/14/20). Browser duration translation continues the sub-slice 20 convention: HOLY_WAR_DECLARATION_DURATION_SECONDS = 18, HOLY_WAR_DURATION_SECONDS = 180, and DIVINE_RIGHT_DECLARATION_DURATION_SECONDS = 180 are reinterpreted as in-world-day numeric values directly rather than translated through DualClock.DaysPerRealSecond. Notable Unity-side departures from browser parity: (1) divine right routes through DynastyOperationLimits even though browser does not gate it on DYNASTY_OPERATION_ACTIVE_LIMIT; this is for surface consistency with all other dispatch consumers and the browser one-active-at-a-time semantic is preserved by the explicit existing-declaration gate; (2) holy war's faith-compatibility gate uses simplified identical-(faith, doctrine) equality instead of browser's getMarriageFaithCompatibilityProfile tier ladder which depends on covenant-name covariance not yet ported (Unity gate is strictly looser than browser); (3) divine right defers covenant-test-passed gate, cooldown gate, stage-ready gate, active-apex-structure gate, recognition-share gate, and faction-kind gate to future slices that port their respective surfaces; (4) divine right defers immediate side effects (mutual hostility application against non-same-faith kingdoms, AI timer cap propagation to candidate factions, conviction event recording) to a future resolution slice. New BloodlinesHolyWarAndDivineRightExecutionSmokeValidation 10-phase validator covers: PhaseHolyWarDispatchSuccess (op created, per-kind attached with ResolveAt = current+18, WarExpiresAt = current+180, IntensityCost=18, EscrowCost.Influence=24, OperatorMemberId resolved, light pulses 0.9/1.2, Influence 50->26, Intensity 30->12, narrative +1 Warn, dispatch cleared), PhaseHolyWarHarmoniousFaithBlocks (identical faith+doctrine blocks), PhaseHolyWarTargetNoFaithBlocks (target CovenantId.None blocks), PhaseHolyWarInsufficientIntensityBlocks (intensity 12 < 18 blocks), PhaseHolyWarInsufficientResourcesBlocks (influence 20 < 24 blocks), PhaseHolyWarDarkPathPulses (dark doctrine produces 1.2/1.8 pulses), PhaseDivineRightDispatchSuccess (op created with default target, per-kind attached with ResolveAt = current+180, SourceFaithId mapped, DoctrinePath captured, recognition/structure placeholders default, narrative +1 Warn, dispatch cleared), PhaseDivineRightInsufficientIntensityBlocks (intensity 50 < 80 blocks), PhaseDivineRightInsufficientLevelBlocks (level 4 < 5 blocks), PhaseDivineRightExistingDeclarationBlocks (preexisting active DivineRight op blocks new creation; only 1 op total). Bundle 2 branched from revision-37 master (0b2fd111) after Bundle 1 landed. No regressions to fortification-siege, captive state, missionary execution, or ai-strategic-layer sub-slice 8-18 surfaces; sub-slice 18 helpers (DynastyOperationLimits.HasCapacity / BeginOperation) gain their second and third production callers without reshape.)
