...
- Last Updated By: codex-unity-fortification-destroyed-counter-recovery-2026-04-20
- Supersedes: revision 40 (Codex fortification-siege sub-slice 9 lands after Claude Bundle 3 advanced master to `e73933e4` and revision 40. This slice starts from the revision-39 breach sealing baseline at `7821d74a`, fast-forwards onto the revision-40 master head after Bundle 3 lands, and advances the contract to revision 41. New `DestroyedCounterRecoveryProgressComponent` plus `DestroyedCounterRecoverySystem` extend the sub-slice 8 sealing loop into slow structural rebuild: a fortified settlement only begins destroyed-counter recovery when `OpenBreachCount == 0`, then reserves stone and accumulates idle-worker hours at 1 Hz toward the highest-priority remaining destroyed counter in strict `Keep > Gate > Wall > Tower` order. Standard rebuilds cost `90` stone and `14` worker-hours per segment; keep rebuilds apply a `2x` multiplier to both cost and hours. On completion the system decrements the chosen destroyed counter and restores the linked destroyed structure's `HealthComponent.Current` to max so `FortificationDestructionResolutionSystem` remains the authoritative source of the destroyed counters on subsequent ticks. The slice stays narrow on purpose: no browser parity surface exists for destroyed-counter recovery, no external HUD or consumer is added, and the live worker scan pattern from sub-slice 8 is reused rather than widening reserve schemas. New `BloodlinesDestroyedCounterRecoverySmokeValidation` covers six phases: progress accumulation after sealing, first wall rebuild completion, keep-over-gate priority with `2x` keep cost and hours, open-breach block, all-counters-zero no-op, and full keep->gate->wall->tower rebuild order with total stone `450`. Governed validation stayed green across the canonical 10-gate chain plus the dedicated destroyed-counter smoke. Because the checked-in bootstrap runtime and scene-shell wrappers still pin `D:\ProjectsHome\Bloodlines\unity`, temporary worktree-safe wrapper copies were used for those gates in the clean merge worktree. Local gitignored Unity-generated csproj files were refreshed before the `dotnet build` gates so they contained both Claude Bundle 3's already-landed files and this slice's new runtime/editor files.)
