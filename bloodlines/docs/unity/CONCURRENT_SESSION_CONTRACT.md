...
- Supersedes: revision 41 (Bundle 4 of the AI mechanical campaign lands on top of Codex's revision-41 fortification-siege sub-slice 9 destroyed-counter recovery. Bundle 4 ships sub-slice 25 missionary resolution alone - the first production per-kind resolution consumer - because per-kind resolution systems differ substantively across operation kinds. AIMissionaryResolutionSystem ([UpdateInGroup(SimulationSystemGroup), UpdateAfter(AIMissionaryExecutionSystem)]) walks every DynastyOperationComponent entity with OperationKind=Missionary and Active=true, filters by DynastyOperationMissionaryComponent.ResolveAtInWorldDays <= current DualClock.InWorldDays, and applies the canonical browser missionary resolution effects from simulation.js:5517-5588 (tickDynastyOperations missionary branch) + simulation.js:10473-10503 (applyMissionaryEffect). Success gate is SuccessScore >= 0 (deterministic, matching browser simulation.js:5527); on success applies ExposureGain to target's FaithExposureElement buffer (appending new entry with Discovered=true if absent, otherwise updating existing entry; clamp [0, 100]), applies IntensityErosion to target's FaithStateComponent.Intensity (clamped >= 0) IFF target has a different committed faith (browser gate simulation.js:10485-10488), applies LoyaltyPressure to lowest-loyalty ControlPointComponent owned by target (clamped [0, 100]); on failure applies +2 intensity reinforcement when target has committed faith (browser simulation.js:5563-5566). Always flips DynastyOperationComponent.Active = false regardless of outcome so per-faction capacity budget (sub-slice 18) releases the slot. Narrative push via NarrativeMessageBridge.Push with three variants (success/failure/void) and canonical tone routing. First consumer of DynastyOperation* per-kind components that reads rather than produces; establishes the resolution walker pattern future per-kind resolution systems (holy war, divine right, captive rescue, captive ransom) can reuse. Unity-side simplifications deferred: ward-profile-triggered mutual hostility on failure; conviction event recording; legitimacy-penalty fallback when target owns zero control points; exposure-threshold crossing narrative. 8-phase BloodlinesMissionaryResolutionSmokeValidation PASS: PhaseNotExpiredSkips; PhaseExpiredSuccessApplies (Active->false, exposure 10->30, intensity 50->45, loyalty 80->77, narrative +1); PhaseExpiredFailureStrengthens (Active->false, target intensity 50->52, exposure/loyalty unchanged); PhaseVoidOnMissingTarget; PhaseExposureAppendsNewEntry; PhaseExposureClampsAt100 (95+20->100); PhaseLowestLoyaltyControlPointTargeted (only 50cp->47, 80cp unchanged); PhaseIntensityErosionSkippedWhenSameFaith (matching faith blocks erosion; intensity remains 50). Rebase note: Bundle 4 originally committed on pre-rewrite master with e73933e4 as direct parent; an upstream history rewrite (collapsing security-hardening + Bundle 3 commits into reindexed equivalents) orphaned that branch from the new master. The Bundle 4 commit was cherry-picked onto the revision-41 master (082699ab) as `claude/unity-ai-missionary-resolution-rebased`; only CONCURRENT_SESSION_CONTRACT.md required conflict resolution, handled by preserving master's revision-41 metadata and amending to revision 42. No regressions to fortification-siege sub-slice 8/9, Bundle 1/2/3, or sub-slice 8-18 surfaces; sub-slice 18 helpers gain a new consumer shape (resolution walker reading DynastyOperationComponent as query surface), sub-slice 20 DynastyOperationMissionaryComponent gains its first production consumer, and sub-slice 16 NarrativeMessageBridge gains three new call sites.)
...
- Current Branch In Flight: `codex/unity-fortification-destroyed-counter-recovery`
- Last Slice Handoff: `docs/unity/session-handoffs/2026-04-20-unity-fortification-siege-destroyed-counter-recovery.md`
...
Note: `fortification-siege-sub-slice-9-destroyed-counter-recovery` is implemented on `codex/unity-fortification-destroyed-counter-recovery` and documented in this revision. The sealing-then-rebuild ordering is now explicit: sub-slice 8 closes live breaches first, and sub-slice 9 only begins slow structural rebuild after `OpenBreachCount` reaches zero. Further fortification follow-ups should continue using fresh `codex/unity-fortification-*` branches rather than widening the earlier fortification branches.
